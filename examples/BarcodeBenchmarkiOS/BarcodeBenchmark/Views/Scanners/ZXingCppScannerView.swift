//
//  ZXingCppScannerView.swift
//  BarcodeBenchmark
//
//  Live camera scanner using ZXing-CPP SDK
//
...
struct ZXingCppScannerView: View {
    @EnvironmentObject var viewModel: BenchmarkViewModel
    @StateObject private var cameraManager = CameraManager()
    @StateObject private var scannerState = ZXingCppScannerState()
    ...
    var body: some View {
        ZStack {
            ...
            BarcodeOverlayView(boxes: scannerState.overlayBoxes, color: .orange, bufferSize: scannerState.bufferSize)
        ...
        .navigationTitle("ZXing-CPP Scanner")
    ...
    private func setupCamera() {
        ...
        do {
            let delegate = ZXingCppCameraDelegate(viewModel: viewModel, state: scannerState)
...
// MARK: - Scanner State
class ZXingCppScannerState: ObservableObject {
...
class ZXingCppCameraDelegate: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    weak var viewModel: BenchmarkViewModel?
    weak var state: ZXingCppScannerState?
    private var lastProcessingTime: Date = .distantPast
    private let processingInterval: TimeInterval = 0.1
    
    init(viewModel: BenchmarkViewModel, state: ZXingCppScannerState) {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        Task {
            do {
                let barcodes = try await viewModel?.zxingcppDetector.detectBarcodes(in: pixelBuffer) ?? []
                await MainActor.run {
