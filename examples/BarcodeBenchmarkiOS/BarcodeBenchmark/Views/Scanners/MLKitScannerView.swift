//
//  MLKitScannerView.swift
//  BarcodeBenchmark
//
//  Live camera scanner using Google ML Kit
//
...
struct MLKitScannerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            BarcodeOverlayView(boxes: scannerState.overlayBoxes, color: .green, bufferSize: scannerState.bufferSize)
    ...
    private func setupCamera() {
        ...
        do {
            let delegate = MLKitCameraDelegate(viewModel: viewModel, state: scannerState)
...
class MLKitCameraDelegate: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    weak var viewModel: BenchmarkViewModel?
    weak var state: ScannerState?
    private var lastProcessingTime: Date = .distantPast
    private let processingInterval: TimeInterval = 0.1
    
    init(viewModel: BenchmarkViewModel, state: ScannerState) {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        Task {
            do {
                let barcodes = try await viewModel?.mlkitDetector.detectBarcodes(in: pixelBuffer) ?? []
                await MainActor.run {
