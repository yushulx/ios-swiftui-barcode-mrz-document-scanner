//
//  DynamsoftScannerView.swift
//  BarcodeBenchmark
//
//  Live camera scanner using Dynamsoft Barcode Reader SDK
//
...
struct DynamsoftScannerView: View {
    ...
    @StateObject private var scannerState = DynamsoftScannerState()
    ...
    var body: some View {
        ZStack {
            ...
            BarcodeOverlayView(boxes: scannerState.overlayBoxes, color: .blue, bufferSize: scannerState.bufferSize)
    ...
    private func setupCamera() {
        ...
        do {
            let delegate = DynamsoftCameraDelegate(viewModel: viewModel, state: scannerState)
...
// MARK: - Scanner State
class DynamsoftScannerState: ObservableObject {
...
class DynamsoftCameraDelegate: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    weak var viewModel: BenchmarkViewModel?
    weak var state: DynamsoftScannerState?
    private var lastProcessingTime: Date = .distantPast
    private let processingInterval: TimeInterval = 0.1
    
    init(viewModel: BenchmarkViewModel, state: DynamsoftScannerState) {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        Task {
            do {
                let barcodes = try await viewModel?.dynamsoftDetector.detectBarcodes(in: pixelBuffer) ?? []
                await MainActor.run {
