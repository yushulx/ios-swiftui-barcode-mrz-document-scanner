...
class ParsedItemModel {
    ...
    func isLegalMRZ(_ parsedItem: ParsedResultItem) -> Bool {
        ...
        var isLegal = true
        if parsedFields["birthDay"] == nil || parsedFields["birthMonth"] == nil
            || parsedFields["birthYear"] == nil || parsedFields["expiryDay"] == nil
            || parsedFields["expiryMonth"] == nil || parsedFields["expiryYear"] == nil
            || parsedFields["sex"] == nil || parsedFields["issuingState"] == nil
            || parsedFields["nationality"] == nil || parsedFields["primaryIdentifier"] == nil
        ...
        if let year = Int(birthYear), let month = Int(birthMonth), let day = Int(birthDay) {
            ...
            if let birthdayDate = birthdayComponents.date {
                ...
                let ageComponents = calendar.dateComponents(
                    [.year, .month, .day], from: birthdayDate, to: currentDate)
    ...
    func isLegalDriverLicense(_ parsedItem: ParsedResultItem) -> Bool {
        ...
        case DriverLicenseType.AAMVA_DL_ID.rawValue:
            if (allKeys.contains("lastName") || allKeys.contains("givenName")
                || allKeys.contains("firstName") || allKeys.contains("fullName"))
        ...
        case DriverLicenseType.AAMVA_DL_ID_WITH_MAG_STRIPE.rawValue:
            if allKeys.contains("name") && allKeys.contains("DLorID_Number") {
                isLegal = true
            }
            break
        case DriverLicenseType.SOUTH_AFRICA_DL.rawValue:
            if allKeys.contains("surname") && allKeys.contains("idNumber") {
                isLegal = true
            }
            break
        default:
...
let AAMVA_DL_ID_InfoList: [[String: String]] = [
...
let AAMVA_DL_ID_WITH_MAG_STRIPE_InfoList: [[String: String]] = [
    ["Title": "Full Name", "FieldName": "name"],
...
let SOUTH_AFRICA_DL_InfoList: [[String: String]] = [
